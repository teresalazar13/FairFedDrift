...
class FairFedDriftAM(Algorithm):
    ...
    def perform_fl(self, seed, clients_data, dataset):
        ...
        clients_identities_printing = [[Identity(global_model.identity.id, global_model.identity.name)] for _ in
                                       range(dataset.n_clients)]
        previous_confidence_clients = [[[WORST_CONFIDENCE for _ in range(len(self.metrics_clustering))]] for _ in
        ...
        test_models(global_models, clients_data[dataset.n_timesteps - 1], clients_metrics, dataset, clients_identities,
                    seed)
    ...
    def update(
        ...
        for client_id, client_data in enumerate(clients_data_timestep):
            ...
            results = test_client_on_model(self.metrics_clustering, global_model_selected.model, dataset.is_binary_target,
                                           client_data[:3])  # client_data[:3] -> x, y, s
