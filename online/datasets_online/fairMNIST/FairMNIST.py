...
class FairMNIST():
    ...
    def create_batched_data(self, _, n_drifts, varying_disc, n_clients, n_rounds):
        drift_ids = self.generate_drift_ids(n_clients, n_rounds, n_drifts)
        (train_X_priv, train_y_priv), (__, _) = mnist.load_data()
        #train_X_priv = train_X_priv[:10000]  # TODO - remove
        #train_y_priv = train_y_priv[:10000]  # TODO - remove
        batched_data = []
        drift_ids_col = [[] for _ in range(n_clients)]
        train_X_priv_rounds = np.array_split(train_X_priv, n_rounds)
        train_y_priv_rounds = np.array_split(train_y_priv, n_rounds)
        ...
        for i in range(n_rounds):
            ...
            for j in range(n_clients):
                ...
                train_X_unpriv_round_client = np.rot90(train_X_priv_round_client * -1, axes=(-2, -1))
                train_y_unpriv_round_client = train_y_priv_round_client
                if drift_id != 0:
                    ...
                    train_y_unpriv_round_client[train_y_unpriv_round_client == drift_id + 2] = 102
                    train_y_unpriv_round_client[train_y_unpriv_round_client == drift_id + 3] = 103
                    ...
                    train_y_unpriv_round_client[train_y_unpriv_round_client == 102] = drift_id + 3
                    train_y_unpriv_round_client[train_y_unpriv_round_client == 103] = drift_id + 2
