...
from federated_online.algorithms.AlgorithmFactory import get_algorithm_by_name
...
def get_arguments():
    ...
    return get_algorithm_by_name(args.fl), get_dataset_by_name(args.dataset), \
           int(args.n_rounds), int(args.n_clients), int(args.n_drifts), float(args.varying_disc)
...
if __name__ == '__main__':
    ...
    clients_data, drift_ids_col, n_features = dataset.create_batched_data(
        algorithm.name, n_drifts, varying_disc, n_clients, n_rounds
    )
    clients_metrics, client_gm_ids_col = algorithm.perform_fl(n_rounds, n_clients, n_features, clients_data, seed, dataset.is_image)

    for i in range(len(clients_metrics)):
        save_results(
            clients_metrics[i], drift_ids_col[i], client_gm_ids_col[i],
            "{}/client_{}/results_online.csv".format(dataset.get_folder(algorithm.name, n_drifts, varying_disc), i+1)
        )
